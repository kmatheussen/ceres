...
void create_transform_expo(struct FFTSound *fftsound){
  struct VarInput *sm;
  struct Vars *sv=malloc(sizeof(struct Vars));

  sv->fftsound=fftsound;

  sm=GUI_MakeMenuButton(fftsound,sv,transMenu,"Exponentiate",0,240);

  GUI_MakeInputVarDouble(sm,"Frequency exponent (0-2): ",&sv->eval2,0.0f,2.0,0.7);
  ...
  GUI_MakeInputVarDouble(sm,"Multiplication factor.:",&sv->mul,0.0f,20.0,1.0);

  GUI_CloseMenuButtonOkCancelPreview(sm,Expo,true);
